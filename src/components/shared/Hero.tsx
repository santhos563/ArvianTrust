...
export function Hero() {
    ...
    return (
        <section className="relative bg-linear-to-br from-[#1F6F5E] to-[#134E44] text-white overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
                <div className="grid md:grid-cols-2 gap-12 items-center">
                    {/* Left Content */}
                    <div className="space-y-6 animate-fade-in">
                        <h2
                            className={`${isTamil ? 'text-2xl md:text-3xl lg:text-4xl' : 'text-3xl md:text-4xl lg:text-5xl'} font-bold leading-tight`}
                            style={{ fontFamily: isTamil ? "'Noto Sans Tamil', 'Inter', sans-serif" : "'Playfair Display', serif" }}
                            dangerouslySetInnerHTML={{ __html: t('hero.title') }}
                        />
                        <p
                            className={`${isTamil ? 'text-base md:text-lg' : 'text-lg md:text-xl'} text-white/90 leading-relaxed`}
                            style={{ fontFamily: isTamil ? "'Noto Sans Tamil', 'Inter', sans-serif" : "'Inter', sans-serif" }}
